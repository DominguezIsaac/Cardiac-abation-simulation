...
int main(){
    ...
    {   
        ...
        {   
            ...
            for (int j = 1; j < Nz; j++)
            {   
                T[j][i]=1/(1+2*gamma)*(T[j][i-1]+dt+gamma*T[j-1][i]+gamma*T[j+1][i]);
            }
        } while (iter<1500 && fabs(T[Nz-1][i]-a)>0.00000000000000001); //amb més precissió el nombre d'iteracions és el mateix
    ...
    output=fopen("CARPETA\\T_implicit(0.5dt).txt","w"); // w de write
